SYSTEM_PROMPT = (
    ...
    "2. Only reference joints, angles, and body parts that appear in metric_status or pose_specific_violations.\n"
...
USER_INSTRUCTIONS = (
    ...
    "- 'pose_specific_violations' (optional): deterministic pose-rule violations. Each item has:\n"
    "    joint: the violated pose-specific area\n"
    "    severity: critical | moderate | minor\n"
    "    feedback: trainer-style correction for that exact violation\n"
    "- 'deterministic_score' (optional): pose-specific deterministic score\n"
    "- 'generic_score' (optional): generic biomechanical score\n\n"
    ...
    "- PRIORITY ORDER: Use pose_specific_violations first when present. Use metric_status as secondary support.\n"
    "- If pose_specific_violations is present, each correction_bullet MUST map to a real violation item.\n"
    "- correction_bullets: ONLY reference real pose_specific_violations (preferred) or metrics marked 'out_of_range' in metric_status. "
    "  Each bullet MUST correspond to real provided evidence. "
    ...
    "- If deterministic_score is provided and >= 90: keep tone strongly positive, with at most one small refinement.\n"
